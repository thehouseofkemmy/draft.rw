...
export type SavedDraft = { body: string; title: string };

type Props = {
  onPublish: (body: string, title: string) => Promise<void>;
  /** Called on every close. Receives the current draft when closing with unsaved text. */
  onClose: (saved?: SavedDraft) => void;
  initialBody?: string;
  initialTitle?: string;
};

export default function ComposeModal({ onPublish, onClose, initialBody = "", initialTitle = "" }: Props) {
  const { user } = useAuth();
  ...
  const { candidates, onTextChange, pickCandidate, dismiss } =
    useMentionAutocomplete({ onInsert: () => {} });

  useEffect(() => {
    const ta = textareaRef.current;
    if (!ta) return;
    // Auto-size on open if pre-filled
  ...
  /** Close and discard — hand text back to parent only if there's something worth keeping. */
  const abandon = () => {
    const body = text.trim();
    onClose(body ? { body: text, title } : undefined);
  };

  useEffect(() => {
    const onKey = (e: KeyboardEvent) => {
      if (e.key === "Escape") {
        if (candidates.length) { dismiss(); return; }
        abandon();
      }
    };
  ...
  }, [candidates.length, dismiss, text, title]);
  ...
  const handlePublish = async () => {
    ...
    setBusy(false);
    onClose(); // published → no draft to save
  };
  ...
  const handleChange = (e: React.ChangeEvent<HTMLTextAreaElement>) => {
    ...
    onTextChange(val, e.target.selectionStart ?? val.length);
  };

  const handlePickMention = (mentionHandle: string) => {
    const ta = textareaRef.current;
    if (!ta) return;
    const newVal = pickCandidate(mentionHandle, text, ta.selectionStart ?? text.length);
    setText(newVal);
  ...
  return (
    <div
      ...
      onClick={(e) => e.target === e.currentTarget && abandon()}
    >
        ...
        <div className="flex items-center justify-between px-5 py-3 border-b border-rule/50 flex-shrink-0">
          <div className="flex items-center gap-2.5">
            ...
            <span className="font-semibold text-[13px] text-ink leading-tight">
              {displayName || handle}
              {handle && <span className="font-mono font-normal text-[11px] text-ink-muted ml-1.5">@{handle}</span>}
            </span>
          </div>
          <button onClick={abandon} className="bg-transparent border-none cursor-pointer text-ink-muted hover:text-ink transition-colors p-1">
            <i className="ti ti-x text-[18px]" />
          </button>
        </div>

        {/* Body */}
        <div className="px-5 pt-4 pb-3 flex flex-col gap-2 relative">
          <input
            type="text"
            value={title}
          ...
          <div className="relative">
            ...
            {candidates.length > 0 && (
              <MentionDropdown
                ...
                onDismiss={dismiss}
                topOffset={textareaRef.current?.scrollHeight}
              />
            )}
          </div>
        </div>
