...
export default function LeftNav({ onAuthOpen }: { onAuthOpen: (mode: "join" | "login") => void }) {
  ...
  return (
    <nav className="flex flex-col gap-0.5 sticky top-0 h-screen py-3 pr-3">
      ...
      <NavItem icon="ti-search"    label="explore"       active={location.pathname === "/search"}         onClick={() => navigate("/search")} />
      <NavItem
        icon="ti-bell"
        label="notifications"
      ...
      <NavItem icon="ti-settings" label="settings"       active={location.pathname === "/settings"}       onClick={() => navigate("/settings")} />
      {isAdmin && (
